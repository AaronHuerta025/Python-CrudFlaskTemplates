...
app.config['MYSQL_DB'] = 'flaskvictor'
...
def index():  # put application's code here
    ...
    return render_template('index.html', edad=edad, dato1="Valor", dato2="Valor2", personas=personas)
...
def contacto(redireccion=None):
    if redireccion is not None:
        ...
        # return redirect('/lenguajes-de-programacion')
...
def editar_coche(coche_id):
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("UPDATE coches SET marca = %s, modelo=%s, precio = %s, ciudad=%s WHERE id = %s",
                       (marca, modelo, precio, ciudad, coche_id))
        cursor.connection.commit()

        flash('El coche se edito')
